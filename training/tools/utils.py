...
class AverageMeter(object):
    ...
    def update_accuracy(self, labels, out_labels):
        current_batch_correct_len = torch.sum(torch.round(sigmoid_layer(out_labels)) == torch.round(labels))
        current_batch_len = len(labels)

        self.sum += current_batch_correct_len
        self.count += current_batch_len
