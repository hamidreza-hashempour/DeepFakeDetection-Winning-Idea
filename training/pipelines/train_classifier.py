...
def main():
    ...
    wandb.tensorboard.patch(root_logdir=args.logdir + '/' + conf.get("prefix", args.prefix) + conf['encoder'] + "_" + str(args.fold))
    from datetime import date

    today = date.today()
    wandb.init(project='DeepFakeClassifier', name="DeepFakeClassifier Run {}".format(today.strftime("%B %d, %Y")), config = 
    {
...
def train_epoch(current_epoch, loss_functions, model, optimizer, scheduler, train_data_loader, summary_writer, conf,
    ...
    for i, sample in pbar:
        ...
        fake_idx = labels > 0.5
        real_idx = labels <= 0.5
        accuracy.update_accuracy(labels, out_labels)
        ohem = conf.get("ohem_samples", None)
        ...
        pbar.set_postfix({"lr": float(scheduler.get_lr()[-1]), "epoch": current_epoch, "loss": losses.avg,
                          "fake_loss": fake_losses.avg, "real_loss": real_losses.avg, "acc": accuracy.acc})
    ...
    if local_rank == 0:
        ...
        summary_writer.add_scalar('train/accuracy', float(accuracy.acc), global_step=current_epoch)
        wandb.log({'train/loss': float(losses.avg) , "train/accuracy": float(accuracy.acc), 'group{}/lr'.format(idx): float(lr)})
